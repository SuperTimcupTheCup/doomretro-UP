...
static void D_DoomMainSetup(void)
{
    ...
    // ----- from beginning and end. this is mostly for correcting TITLEPIC and CREDITS so they can function properly. also offering widescreen versions of those lumps if available, and correcting the title and credits for certain IWADs that have them in non-standard lumps. this is also used to determine if the game is running with the Unity assets, which affects the title and credits used as well as some other things.
    if (chex)
    {
        titlelump = W_CacheLumpName("TITLECHE");
        creditlump = W_CacheLastLumpName("CREDIT4");
    }
    else if (harmony)
    {
        titlelump = W_CacheLumpName("TITLEHAR");
        creditlump = W_CacheLastLumpName("CREDIT4");
    }
    else if (hacx)
    {
        titlelump = W_CacheLumpName("TITLHACX");
        creditlump = W_CacheLastLumpName("CREDIT5");
    }
    else if (autosigil)
    {
